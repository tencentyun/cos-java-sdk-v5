 ...
 * <p>
 * This class is obtained with the {#link {@link #getInstance()} method that recognizes loaded mime
 ...
 * <code>mimetype <Space | Tab>+ extension (<Space|Tab>+ extension)*</code>. Any blank lines in the
 * file are ignored, as are lines starting with <code>#</code> which are considered comments. Lines
 * that have a mimetype but no associated extensions are also ignored.
 */
public class Mimetypes {
    ...
    /**
     * Reads and stores the mime type setting corresponding to a file extension, by reading text
     * from an InputStream. If a mime type setting already exists when this method is run, the mime
     * type value is replaced with the newer one.
     *
     * @param is
     *
     * @throws IOException
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (line.startsWith("#") || line.length() == 0) {
                ...
                if (st.countTokens() > 1) {
                    ...
                    log.debug(
                            "Ignoring mimetype with no associated file extensions: '" + line + "'");
                }
            }
        }
    }
     ...
     * <p>
     * A file extension is one or more characters that occur after the last period (.) in the file's
     * name. If a file has no extension, Guesses the mimetype of file data based on the file's
     * extension.
     *
    ...
    public String getMimetype(String fileName) {
        ...
        if (lastPeriodIndex > 0 && lastPeriodIndex + 1 < fileName.length()) {
            ...
            log.debug(
                    "File name has no extension, mime type cannot be recognised for: " + fileName);
        }
